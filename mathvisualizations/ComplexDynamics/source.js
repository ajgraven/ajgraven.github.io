...
class FractalPlot {
  constructor(varName, paramDict, canvasName, canvasID, callbacks = {}, fractType = "dyn", canvasWidth = 500, canvasHeight = 500) {
    ...
    this._keydownscript = 'keyval = key();' +
          'if(keyval=="\'",keyval="l");' +
          'javascript("' + this._varName + '.keypress(\'"+keyval+"\')");';
    this._cindy = CindyJS({
  ...
  keypress(key) {
    switch(key) {
      case "»":  this.zoomIn(2); break;
      case "½":  this.zoomIn(1/2); break;
      case "&":  this.shift([0,1/(this._zoom*4)]); break;
      case "(":  this.shift([0,-1/(this._zoom*4)]); break;
      case "l":  this.shift([1/(this._zoom*4),0]); break;
      case "%":  this.shift([-1/(this._zoom*4),0]); break;
    }
  }
...
var apply_preset = function(preset_val) {
  document.getElementById("inpparamcenter").value = param_preset_dict[preset_val].center;
  document.getElementById("inpparamzoom").value = param_preset_dict[preset_val].zoom;
  document.getElementById("inpdyncenter").value = dyn_preset_dict[preset_val].center;
  document.getElementById("inpdynzoom").value = dyn_preset_dict[preset_val].zoom;
  document.getElementById("inpc").value = param_preset_dict[preset_val].c;
  document.getElementById("inpn").value = param_preset_dict[preset_val].n;
  document.getElementById("inpf").value = param_preset_dict[preset_val].f;
  document.getElementById("inpme").value = param_preset_dict[preset_val].escape;
  document.getElementById("inpje").value = dyn_preset_dict[preset_val].escape;
